...
Private Type ParsedInput
    ...
    HasP3 As Boolean
    P3PersonId As Long
    P3Times(1 To 5) As String
    P3TimesBlocked(1 To 5) As Boolean
    RedconCount As Long
    ' Note: RedconList and People are stored as module-level variables
...
' Module-level People database — alpha manning OICs + P3 activator.
...
'====================================================
'CLIPBOARD STRING FORMAT (version 2)
'
...
'  6  YES | NO                  (P3 flag)
'  7  P3 person ID              (blank if no P3)
'  8  P3 time 1 (HHMM)         (blank if no P3)
'  9  P3 time 2
'  10 P3 time 3
'  11 P3 time 4
'  12 P3 time 5
'  13 REDCON count
'  14+ A4xx code, person ID     (2 fields per appliance)
'
...
Private Sub ParseClipboardString(ByVal raw As String, ByRef result As ParsedInput)
    ...
    If Trim(parts(1)) <> "2" Then
        result.ErrorMsg = "This code was generated by an older version of the control panel." & vbCrLf & _
                          "Please refresh the control panel and regenerate the code."
        Exit Sub
    End If

    ' ================================================================
    ' PRE-PASS: scan every token from index 14 onward, looking for the
    ' PEOPLE and ROTA_PEOPLE section headers.  Load them into
    ' m_People[] / m_RotaPeople[] BEFORE the REDCON section is parsed.
    '
    ' This is the fix for the "ID NOT FOUND" bug: REDCON entries call
    ' LookupPersonById(), which needs m_People[] already populated.
    ' In the old single-pass code the PEOPLE section came AFTER REDCON,
    ' so m_People[] was empty at lookup time.
    ' ================================================================
    Dim ps As Long
    For ps = 14 To UBound(parts)
    ...
    ' m_People[] and m_RotaPeople[] are now fully populated, so
    ' LookupPersonById() calls inside the REDCON loop will succeed.
    ' ================================================================
    ...
    result.HasP3 = (UCase(Trim(parts(6))) = "YES")

    If result.HasP3 Then
        If Not IsNumeric(Trim(parts(7))) Then
            result.ErrorMsg = "Invalid P3 person ID in code."
            Exit Sub
        End If
        result.P3PersonId = CLng(Trim(parts(7)))
        Dim pti As Integer
        For pti = 1 To 5
            Dim ptVal As String
            ptVal = Trim(parts(7 + pti))
            If UCase(ptVal) = "BLOCKED" Then
                result.P3TimesBlocked(pti) = True
                result.P3Times(pti) = ""
            Else
                result.P3TimesBlocked(pti) = False
                result.P3Times(pti) = ptVal
            End If
        Next pti
    End If

    If Not IsNumeric(Trim(parts(13))) Then
        result.ErrorMsg = "Invalid REDCON count in code."
        Exit Sub
    End If

    result.RedconCount = CLng(Trim(parts(13)))
    m_RedconCount = result.RedconCount

    If m_RedconCount > 0 Then
        If UBound(parts) < 13 + m_RedconCount * 2 Then
...
Private Sub BuildDayReport( _
    ...
    AddP3RowsFromData rows, n, parsed
...
'P3 FROM PARSED DATA
'====================================================
Private Sub AddP3RowsFromData(ByRef rows() As ReportRow, ByRef n As Long, ByRef parsed As ParsedInput)

    If Not parsed.HasP3 Then Exit Sub

    Dim activator As String
    activator = UCase(FormatPersonName(LookupPersonById(parsed.P3PersonId)))

    Dim p3Labels(1 To 5) As String
    p3Labels(1) = "P3 Activated by " & activator
    p3Labels(2) = "P3 Left Division"
    p3Labels(3) = "P3 Arrive At Location"
    p3Labels(4) = "P3 Left Location"
    p3Labels(5) = "P3 Reach Division"

    Dim pi As Integer
    For pi = 1 To 5
        If Not parsed.P3TimesBlocked(pi) And parsed.P3Times(pi) <> "" Then
            AddRow rows, n, parsed.P3Times(pi), "-", "-", "-", _
                p3Labels(pi), parsed.P3Times(pi), "All in order.", True, False
        End If
    Next pi
